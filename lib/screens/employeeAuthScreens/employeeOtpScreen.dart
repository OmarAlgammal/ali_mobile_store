...
class EmployeeOTPScreen extends StatefulWidget {
  ...
  EmployeeOTPScreen({this.phoneNum, this.verificationId});
    ...
    return BlocBuilder<UserAuthBloc, UserAuthState>(
      builder: (context, state) {
        if (state is InvalidEmployeeOtpNum){
          BlocProvider.of<UserAuthBloc>(context).add(EmployeeCodeSentEvent(phoneNum: widget.phoneNum, verificationId: widget.verificationId));
        ...
        return InkWell(
          onTap: (){
            setState(() {
              ...
              BlocProvider.of<UserAuthBloc>(context).add(VerifyEmployeeOtpNumEvent(context: context,phoneNum: widget.phoneNum, otp: _otp, verificationId:  widget.verificationId));
            });
          },
          child: Padding(
